...
public class Compression : MonoBehaviour
{
    ...
    public int StiffnessMultiplier = 1;
    public int DampingMultiplier = 1;
    public float offset;
        ...
        for (int h = 0; h < LayerNumber; h++)  // Generate Layer
        {
            ...
            for (int x = 0; x < gridSize.x; x++)    // Generate Row
            {
                ...
                for (int y = 0; y < gridSize.y; y++)    // Generate Column
                {
                    ...
                    go.transform.position = new Vector3(x * (0.01f + VoxelSize.x), (h + 2) * VoxelSize.y+offset, y * (0.01f + VoxelSize.z));
                    ...
                    if (h == 0)     // If bottom layer, anchor it to offseted ground
                    {
                        spring.connectedAnchor = new Vector3(x * (0.01f + VoxelSize.x), offset, y * (0.01f + VoxelSize.z));
                    }
                    else
                    ...
                    if (x == nod_x && y == nod_y)
                    {
                        for (int i = -NoduleSize; i <= NoduleSize; i++) {
                            for (int j = -NoduleSize; j <= NoduleSize; j++)
                            {
                                ...
                                spring = go.GetComponent<SpringJoint>();
                                spring.spring = SpringConstant * StiffnessMultiplier;
        ...
        for (int h = 0; h < LayerNumber; h++)  // Generate Layer
        {
            ...
            for (int x = 0; x < gridSize.x; x++)    // Generate Row
            {
                ...
                for (int y = 0; y < gridSize.y; y++)    // Generate Column
                {
                    ...
                    GameObject go = GameObject.CreatePrimitive(PrimitiveType.Cube);
                    go.transform.localScale = VoxelSize;
                    go.transform.position = new Vector3(x * (LayerDistance + VoxelSize.x), (h + 2) * VoxelSize.y + offset, y * (LayerDistance + VoxelSize.z));
                    string x_label = CreateLabel(x);
                    string y_label = CreateLabel(y);
                    go.name = h_label + x_label + y_label;
                    ...
                    if (h == 0)     // If bottom layer, anchor it to offseted ground
                    {
                        spring.connectedAnchor = new Vector3(x * (LayerDistance + VoxelSize.x), offset, y * (LayerDistance + VoxelSize.z));
                    }
                    else
                    {      // Otherwise anchor it to the voxel below it
                        string h_below = CreateLabel(h - 1);
            ...
            for (int x = 0; x < gridSize.x; x++)
            {
                for (int y = 0; y < gridSize.y; y++)
                {
                    ...
                    if (x <= gridSize.x - 2 & y <= gridSize.y - 2)
                    {
                        ...
                        springjoint_x.spring = SpringConstant;
                        ...
                        springjoint_y.spring = SpringConstant;
